...
# counter originally start from 1 
cnt = 1
server.add_handler('_out') do |tuple|
    # check if tuple has be generated befor, just write tuple into tuplespace
    if tuple.length == 4
        ts.write map_symbols_in(tuple)
        nil
    else
        # insert sequential number into tuple  and then write into tuplespace
        tuple.push(cnt)
        puts "current counter "+ String(cnt)
        ts.write map_symbols_in(tuple)
        cnt += 1
        nil
    end
